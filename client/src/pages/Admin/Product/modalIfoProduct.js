...
function ModalInfoProduct({ product, changeStatus }) {
    return (
        <div className='w-full h-auto px-8 pb-8 pt-4'>
            <h2 className="font-extrabold text-4xl text-center text-black">Thông tin chi tiết sản phẩm</h2>
            <div className="mt-8 space-y-4">
                <div className="w-full flex gap-x-2">
                    ...
                    <div className="w-5/6 flex justify-between gap-x-2 -mt-2">
                        ...
                        <div className=" space-y-2">
                            ...
                            <p className="p-2 rounded-md bg-white">
                                ...
                                <span>{product.sale_price.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</span>
                            </p>
                            <p className="p-2 rounded-md bg-white">
                    ...
                    <div className="w-full flex justify-center gap-x-2">
                        {
                            product.photo_detail.map(ptDetail =>
                                <Image src={`${process.env.REACT_APP_API_URL}/api/products/${product._id}/${ptDetail}`}
                                    alt='ảnh vô cực'
                                    className='w-1/5 h-auto rounded object-cover' />
                            )
                        }
                    </div>
                </div >
